export function generateContent(input) {
  return {
    shortVideoScripts: Array.from({ length: 10 }, (_, index) => `Script ${index + 1}: Hook with the pain of scattered execution, show how ${input.name} organizes strategy/content/CRM in one flow, and end with a CTA to request a demo.`),
    socialPosts: [
