export async function scrapeSource(sourceType, sourceValue, description) {
  const normalized = `${sourceType}: ${sourceValue} ${description}`.trim();
  const keywords = normalized.replace(/https?:\/\//g, '').split(/[^\p{L}\p{N}]+/u).filter(Boolean).slice(0, 20);
  return {
